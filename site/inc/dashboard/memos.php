...
$query = "SELECT * FROM `memo_crm` WHERE (`receivers` LIKE'%\"$userinfo->email\"%'  AND `token`='$memoToken2');" ;
...
if (!empty($memo) && in_array($sitePage, array('memos'))) {
	...
	if (!in_array($userinfo->email, $memo->readers)) {
		...
		$ezDb->query("UPDATE `memo_crm` SET `readers`='".json_encode($memo->readers)."' WHERE `token`='$memoToken2';");
	}
	$smarty->assign("memo_message", $memo);
}else{
	$memos=$ezDb->get_results("SELECT * FROM `memo_crm` WHERE (`id`!='0' AND `receivers` LIKE'%\"$userinfo->email\"%') ORDER BY `id` DESC ");
