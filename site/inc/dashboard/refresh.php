<?php
	...
	if($posts->sendMode == 'getFresh'){
		...
		$memos_dash=$ezDb->get_results("SELECT * FROM `memo_crm` WHERE `id`!='0' $whereClause ORDER BY `id` DESC LIMIT 5 ");
		...
		//messages
		$whereClause="AND `receivers` LIKE'%\"$userinfo->email\"%' AND `readers` NOT LIKE'%\"$userinfo->email\"%'";
		$messagesbar=tableRoutine("messages_crm", '*', " `id`!='0' $whereClause", '`id`');
		$count = 0;
