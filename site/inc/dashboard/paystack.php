...
if (!empty($sessions->token) && in_array($sitePage, array("paystack")) && ($requestMethod == 'POST') && !empty($posts->payBtn) && $posts->payBtn=='paystack') {
	if (!empty($servers->HTTP_REFERER)) {
		...
		if($sessions->purpose=='investment'){
			...
			$property=$ezDb->get_row("SELECT * FROM `projects` WHERE `token`='$subscription->project_token'");
		}
		...
		if(!empty($err) and $err){
			...
			$Site["session"]['amount']=$amount/100;
			//$Site["session"]['pslug']=$Site["session"]['purpose']= $subscription->product.' subscription';
			$Site["session"]['discount']=$discount;
			$_SESSION=$Site["session"];
			$sessions= (object)$Site["session"];

			$ezDb->query("UPDATE `payments` SET `amount`='$sessions->amount' WHERE `token`='$sessions->token' AND `transid1` = '$sessions->transid';");
