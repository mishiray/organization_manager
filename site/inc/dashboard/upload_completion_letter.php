...
if ( !empty($image_base64) and file_put_contents($targetFile, $image_base64)) :
	...
	if(!empty($sub)){
		...
		$title = 'Completion letter for '.$sub->first_name.' '.$sub->last_name.' on '.$sub->product;
		$description = 'Your completion letter for '.$sub->product;
		$ezDb->query("INSERT INTO `pdocuments` (`reftoken`,`token`,`title`, `category`, `description`,`file`,`status`,`addedby`,`active`) VALUES ('$sub->token','$token','$title','completion_letter','$description','$targetFile',0,'$userinfo->email',1)");
