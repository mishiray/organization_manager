...
$filter = new stdClass();
if (in_array($sitePage, array("agriculture")) && ($requestMethod == 'POST') && $posts->triggers=='search_property') {
...
$projects=tableRoutine("agriculture", '*' , " `id`!=0 $whereClause", '`id`', 'DESC', '`id`', '8');
if(!empty($projects)){
	foreach($projects as $pro){
		
		$pro->land = json_decode($pro->land);
		$pro->lease = json_decode($pro->lease);
		$pro->landr = json_decode($pro->landr);

	}
}
$smarty->assign("agr_pro", $projects)->assign("filter", $filter);