...
$filter = new stdClass();
if (in_array($sitePage, array("weekly_reports")) && ($requestMethod == 'POST') && $posts->triggers=='reports') {
	//$err=0;
	$whereClause = " ";
	...
	$weekly_report=$ezDb->get_results("SELECT * from `weekly_report` WHERE `id`!=0 $whereClause ORDER BY `id` DESC");
		if(!empty($weekly_report)){
			foreach ($weekly_report as $value) {
				$value->name = $ezDb->get_row("SELECT `surname`, `first_name` FROM `employees` WHERE `employeeid` = '$value->employeeid'");
			}
		}	
	$smarty->assign("weekly_report", $weekly_report)->assign("months", $months)->assign("filter",$filter);
}else{
	$whereClause=(
		$userinfo->userrole=='level3' ? "AND `user`='$userinfo->email' AND `status` IN('0','1','3', '2', '4')": (
			$userinfo->userrole=='level2'? "AND `status` IN('0', '1', '3', '4', '2')": (
